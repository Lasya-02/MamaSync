...
export default function Reminder() {
    ...
    const checkReminders = () => {
      ...
      reminders.forEach((reminder) => {
        ...
        notificationMinutes.forEach((targetMinutes) => {
          ...
          if (
            differenceFromTarget <= 30000 &&
            isAtOrPastTargetTime &&
            !reminder[notifKey]
          ) {
            if (
              ...
              let notifMessage = "";
              if (targetMinutes === 0) {
                notifMessage = `${reminder.title}${
                  reminder.description ? " - " + reminder.description : ""
                } NOW!`;
              } else {
                notifMessage = `${reminder.title}${
                  reminder.description ? " - " + reminder.description : ""
                } in ${targetMinutes} minutes`;
              }
              ...
              try {
                ...
                const notif = new Notification("🔔 MamaSync Reminder", {
                  body: notifMessage,
              ...
              setReminders((prev) =>
                prev.map((r) =>
                  r.id === reminder.id ? { ...r, [notifKey]: true } : r
                )
              );
            }
          }
  ...
  const upcomingReminders = sortedReminders.filter(
    (r) => new Date(`${r.date}T${r.time}`) > currentTime
  );
      ...
      {inAppNotification && (
        ...
        >
          ...
          <div style={{ display: "flex", gap: "0.5rem" }}>
            <button
              onClick={() => handleDismissNotifications(inAppNotification.id)}
              style={{
                    ...
                    <div key={reminder.id} className="reminder-card upcoming">
                      ...
                      <div className="reminder-content">
                        ...
                        <div className="reminder-meta">
                          ...
                          <span
                            className={`reminder-category category-${reminder.category}`}
                          >
                            {reminder.category}
                          </span>
